  ...
  try {
    ...
    for (const studentData of PAYMENT_DATA) {
      ...
      const [students] = await conn.query(
        "SELECT s.id as student_id, se.id as enrollment_id FROM students s JOIN student_enrollments se ON s.id = se.student_id WHERE LOWER(TRIM(s.full_name)) = LOWER(TRIM(?)) AND s.deleted_at IS NULL AND se.deleted_at IS NULL LIMIT 1",
        [studentData.name]
      );
      ...
      for (const paymentAmount of studentData.payments) {
        ...
        await conn.query(
          ...
          [student.id, student.enrollment_id, transactionId, paymentAmount, transactionRef, `Payment received from ${studentData.name}`]
        );
