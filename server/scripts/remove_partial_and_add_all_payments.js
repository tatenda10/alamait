...
async function removePartialAndAddAll() {
  const conn = await db.getConnection();
  try {
    console.log('Step 1: Removing partial payments and resetting balances...\n');
    
    await conn.beginTransaction();
    
    // 1. Remove all payment transactions I added earlier
    console.log('Removing partial payment transactions...');
    ...
    `);
    console.log(`Removed ${deletedTransactions.affectedRows} payment transactions`);
    
    // 2. Remove associated journal entries
    console.log('Removing associated journal entries...');
    ...
    `);
    console.log(`Removed ${deletedJournals.affectedRows} journal entries`);
    
    // 3. Remove student_payments records
    console.log('Removing student_payments records...');
    const [deletedStudentPayments] = await conn.query(`
      UPDATE student_payments 
      SET deleted_at = NOW()
      WHERE deleted_at IS NULL
    `);
    console.log(`Removed ${deletedStudentPayments.affectedRows} student payment records`);
    ...
    // 5. Verify balances are back to original state
    ...
    console.log(`Cash balance: $${cashBalance[0]?.current_balance || 0}`);
    console.log(`AR balance: $${arBalance[0]?.current_balance || 0}\n`);
    
    // Step 2: Add ALL payments from the image
    console.log('Step 2: Adding ALL payments from the image...\n');
    ...
    console.log(`Processing ${PAYMENT_DATA.length} students with ALL payments from the image...\n`);
    ...
    // Final recalculation
    console.log('\nFinal recalculation of account balances...');
    await recalculateAllAccountBalances();
    console.log('✅ Account balances recalculated');
