...
const BANK_TRANSACTIONS = [
  ...
  // Rent transfers from Cash to Bank
  {
    date: '2025-08-30',
    type: 'cash_to_bank',
    description: 'Rent',
  ...
  // Expenses paid from Bank
  {
    ...
    description: 'Wifi',
    amount: 100.00,
    debitAccount: '50026', // Utilities - wifi
  ...
  },
  {
    date: '2025-09-26',
    type: 'expense',
    description: 'Meadow and Willow - Rental Expense',
    amount: 1500.00,
    debitAccount: '50013', // Administrative Expenses
    creditAccount: '10003' // CBZ Bank Account
  }
...
async function main() {
  ...
  try {
    ...
    const accountIds = {};
    const codes = ['10002', '10003', '30004', '50013', '50026'];
    ...
    console.log('\nStep 5: Processing Bank transactions...');
    let transactionCount = 0;
    let totalRentTransfers = 0;
    let totalExpenses = 0;
    ...
    for (const txn of BANK_TRANSACTIONS) {
      ...
      transactionCount++;
      const txRef = `BANK-STK-${txn.date}-${transactionCount}`;
      ...
      // Track totals
      if (txn.type === 'cash_to_bank') {
        totalRentTransfers += txn.amount;
    ...
    console.log(`Total rent transfers (Cash → Bank): $${totalRentTransfers.toFixed(2)}`);
    console.log(`Total expenses paid from Bank: $${totalExpenses.toFixed(2)}`);
    console.log(`\nPrevious Bank balance: $${bankBalance?.current_balance || 0}`);
    console.log(`Final Bank balance: $${finalBank?.current_balance || 0}`);
    console.log(`\nPrevious Cash balance: $${cashBalance?.current_balance || 0}`);
    console.log(`Final Cash balance: $${finalCash?.current_balance || 0}`);
    
    console.log('\n✅ Bank transaction import completed successfully!');
