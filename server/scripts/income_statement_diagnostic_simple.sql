-- Complete Income Statement Diagnostic Query (Fixed Version)
...
SELECT 
  ...
  CONCAT('Total: ', COUNT(*), ' | Posted: ', COALESCE(COUNT(CASE WHEN status = 'posted' THEN 1 END), 0), 
         ' | Draft: ', COALESCE(COUNT(CASE WHEN status = 'draft' THEN 1 END), 0),
         ' | Voided: ', COALESCE(COUNT(CASE WHEN status = 'voided' THEN 1 END), 0),
         ' | NULL: ', COALESCE(COUNT(CASE WHEN status IS NULL THEN 1 END), 0)) as account_name,
  COUNT(*) as amount,
...
FROM (
  SELECT 
    t.id as account_id,
    coa.code as account_code,
    CONCAT('Txn: ', t.id, ' | Type: ', COALESCE(t.transaction_type, 'NULL'), ' | Date: ', t.transaction_date, 
           ' | Status: ', COALESCE(t.status, 'NULL'), ' | Amount: $', je.amount) as account_name,
    je.amount as amount,
    NULL as transaction_count,
    bh.name as boarding_house_name,
    COALESCE(t.status, 'NULL') as transaction_status,
    COALESCE(LEFT(t.description, 50), 'No description') as debug_info
  FROM journal_entries je
  JOIN transactions t ON je.transaction_id = t.id
  JOIN chart_of_accounts coa ON je.account_id = coa.id
  JOIN boarding_houses bh ON je.boarding_house_id = bh.id
