...
async function moveStudentsFromTestToStKilda() {
  ...
  try {
    ...
    try {
      ...
      // Define the room name mappings from test to St Kilda
      const roomMappings = {
        'Bus 1': 'BUS1',
        'Bus 2': 'BUS2', 
        'Ext 1': 'EXT1',
      ...
      console.log('\n🔄 Moving students from test rooms to St Kilda rooms...');
      
      let movedCount = 0;
      
      for (const [testRoomName, stKildaRoomName] of Object.entries(roomMappings)) {
        console.log(`\n🏠 Moving students from ${testRoomName} → ${stKildaRoomName}:`);
        
        // Find the St Kilda room ID
        const [stKildaRoom] = await connection.execute(`
          SELECT id, name FROM rooms 
          WHERE name = ? AND boarding_house_id = ? AND deleted_at IS NULL
        `, [stKildaRoomName, stKildaId]);
        
        if (stKildaRoom.length === 0) {
          console.log(`  ❌ St Kilda room ${stKildaRoomName} not found`);
          continue;
        }
        
        const stKildaRoomId = stKildaRoom[0].id;
        console.log(`  ✅ Found St Kilda room ${stKildaRoomName} (ID: ${stKildaRoomId})`);
        
        // Find students enrolled in the test room
        ...
        console.log(`  👥 Found ${studentsToMove.length} students in test room ${testRoomName}`);
        
        // Move students to St Kilda room
        for (const student of studentsToMove) {
          await connection.execute(`
            UPDATE student_enrollments 
            SET room_id = ?
            WHERE id = ?
          `, [stKildaRoomId, student.enrollment_id]);
          
          console.log(`    ✅ Moved ${student.full_name} to St Kilda ${stKildaRoomName}`);
          movedCount++;
        }
      }
      ...
      console.log('\n🎉 All 58 students moved to correct St Kilda rooms and assigned to beds!');
